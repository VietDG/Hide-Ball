...
public static class FunctionCommon
{
    ...
    public static Tweener ChangeValueFloat(float startValue, float endValue, float speed, float delay,
        Action<float> onUpdate, TweenCallback onComplete = null)
    {
        return DOTween.To(() => startValue, x => startValue = x, endValue, speed).
            OnUpdate(delegate
            {
                onUpdate?.Invoke(startValue);
            }).SetEase(Ease.Linear).SetDelay(delay).OnComplete(onComplete);
    }
