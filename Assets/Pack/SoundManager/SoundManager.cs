...
public class SoundManager : SingletonMonoBehaviour<SoundManager>
{
    ...
    private void Start()
    {
        ...
        if (PlayerPrefs.GetInt(Const.FIRST_TIME_OPEN) == 0)
        {
            ...
            PlayerPrefs.SetFloat(Const.MUSIC_VALUE, 0f);
            PlayerPrefs.SetFloat(Const.SOUND_VALUE, 0f);
    ...
    #region On Vibration
    public void PlayVibration(long milliseconds)
    {
        if (!Setting.VIBRATE) return;
    ...
    public void HandleVolumSound(AudioClip sfx, float value)
    {
        if (playingSound.ContainsKey(sfx.GetInstanceID()))
        {
            playingSound[sfx.GetInstanceID()].volume = value;
        }
    }
}
